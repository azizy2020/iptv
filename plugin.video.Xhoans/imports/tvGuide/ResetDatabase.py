...
def deleteDB():
    try:
        xbmc.log("[plugin.video.Xhoans] Deleting database...", xbmc.LOGDEBUG)
        dbPath = xbmc.translatePath(xbmcaddon.Addon(id = 'plugin.video.Xhoans').getAddonInfo('profile'))
        ...
        if passed:
            xbmc.log("[plugin.video.Xhoans] Deleting database...PASSED", xbmc.LOGDEBUG)
        else:
            xbmc.log("[plugin.video.Xhoans] Deleting database...FAILED", xbmc.LOGDEBUG)

        return passed

    except Exception, e:
        xbmc.log('[plugin.video.Xhoans] Deleting database...EXCEPTION', xbmc.LOGDEBUG)
        return False
